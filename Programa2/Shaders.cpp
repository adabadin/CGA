...
void CShader::BindVariable(string strVariable, const float* value, size_t size)
{
   if(m_hProgramObject)
   {
		...
		if      (size == 1)  glUniform1fARB(location, value[0]);
		else if (size == 2)  glUniform2fARB(location, value[0], value[1]);
		else if (size == 3)  glUniform3fARB(location, value[0], value[1], value[2]);
		/*
